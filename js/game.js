...
function handleCardClick(card, index) {
    const cardSound = document.getElementById('flipSound');
    ...
    // Play flip sound
    if (cardSound) {
        cardSound.play().catch(error => console.log('Sound play failed:', error));
    }
    ...
    if (gameState.flippedCards.length === 2) {
        ...
        if (match) {
            ...
            setTimeout(() => {
                ...
                if (mismatchSound) {
                    mismatchSound.play().catch(error => console.log('Sound play failed:', error));
                }
...
function handleWin() {
    ...
    if (winSound) {
        winSound.play().catch(error => console.log('Sound play failed:', error));
    }
